...
def conv_bn(inputs, filters, kernel_size, name, training, skip=None):
...
def res_block(inputs, filters, kernel_size, name, training):
    with tf.variable_scope(name):
        ...
        conv = conv_bn(conv,   filters, kernel_size, "conv_2", training, inputs)
    return conv
...
def export_meta(model_name):
    with tf.Graph().as_default():
        ...
        saver.export_meta_graph(os.path.join(model_name, model_name + ".meta"))
