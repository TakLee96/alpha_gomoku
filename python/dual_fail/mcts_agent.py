...
class MCTSAgent(DualAgent):
    ...
    def get_dist(self, state):
        ...
        # print("v: %f" % self.get_value(state))
        return count

    def get_value(self, state):
        # return a neutral representation of current value [-1, 1]
        v = state.player * self.tree.get_value()
        if v > 1:
            return 1
        elif v < -1:
            return -1
...
class Node:
    ...
    def grow(self, sess, state, depth, epsilon, multiplier):
        ...
        if len(state.history) == 0:
            ...
            U = self.Q + multiplier * self.P * const / (1 + self.N)
            U[(state.board != 0).reshape(225)] = -10 * INFINITY           # never visit illegal positions
            a = U.argmax()
