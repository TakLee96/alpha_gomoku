...
with b_sess.as_default():
    with b_graph.as_default():
        b_agent = MCTSAgent(b_sess, args.model_name_2, chkpnt=args.chkpnt2, epsilon=args.epsilon)
print("ARENA: DUAL %s-%d VERSES MCTS %s-%d" % (a_agent.model_name, a_agent.chkpnt, b_agent.model_name, b_agent.chkpnt))
