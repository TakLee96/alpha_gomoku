...
def get_random_mcts_action(agent, state):
    dst = agent.get_dist(state)
    dst = dst * dst
    dst = dst / dst.sum()
    dst[(state.board != 0).reshape(225)] = 0
    c = np.random.choice(225, p=dst)
    x, y = np.unravel_index(c, dims=(15, 15))
    assert state.board[x, y] == 0, "total prob %f that prob %f" % (dst.sum(), dst[c])
    return x, y
...
with current_sess.as_default():
    with current_graph.as_default():
        agent = MCTSAgent(current_sess, "treesup", "dualdagger", epsilon=0.2, multiplier=3)
