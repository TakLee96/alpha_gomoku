...
class MinimaxAgent:
    ...
    def _max_value(self, state, alpha, beta, depth):
        ...
        for x, y in actions:
            ...
            max_value = max(max_value,
                DISCOUNT * self._min_value(state, alpha, beta, depth + 1))
            if max_value > beta:
                state.rewind()
                return max_value
    ...
    def _min_value(self, state, alpha, beta, depth):
        ...
        for x, y in actions:
            ...
            min_value = min(min_value,
                DISCOUNT * self._max_value(state, alpha, beta, depth + 1))
            if min_value < alpha:
                state.rewind()
                return min_value
    ...
    def get_score(self, state):
        ...
        def evaluate(action):
            ...
            state.move(x, y)
            if who == 1:
                val = self._min_value(state, -INFINITY, INFINITY, 1)
            else:
                val = self._max_value(state, -INFINITY, INFINITY, 1)
            state.rewind()
            return x, y, val
        return list(Pool().imap_unordered(evaluate, actions))
